  ...
  const performSentimentAnalysis = async () => {
    if (file) {
      ...
      // Log the file and the FormData object after appending
      console.log("File:", file);
      console.log("FormData:", formData.get("file")); 
      console.log("File exists in FormData:", formData.has("file"));
      ...
      const response = await fetch("/api/uploads", {
        ...
        .then((data) => {
          // displayAlert(data.message); // Pass the content to the displayAlert function
  ...
  const onFileDrop = (e) => {
    ...
    console.log(newFile); // Check the newFile object
  
    if (newFile && newFile.name) {
      setFile(newFile); // Set the file state directly
    }
  };
